...
* Projects live in `projects/` and CLI tools/scripts live in `tools/`. Each subdirectory is self-contained with its own dependencies and configuration files.
...
### Positive
* **Decoupling**: Adding, modifying, or removing a project under `projects/` or `tools/` will not break dependencies in other projects.
